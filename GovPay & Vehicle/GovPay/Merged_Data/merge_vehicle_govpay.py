...
OUTPUT_FILE = '/Users/user/Desktop/Korlie_Data/Merged_Data/vehicle_govpay_merged_by_amount.csv'
...
def main():
    print("=" * 60)
    print("MERGING VEHICLE RENEWAL DATA WITH GOVPAY DATA")
    print("=" * 60)
    ...
    # Note: Multiple rows may have the same amount, so we store as list
    govpay_by_amount = {}
    for row in govpay_rows:
        if len(row) > govpay_amount_idx:
            amount = row[govpay_amount_idx].strip()
            # Normalize amount (remove .0 if present for matching)
            try:
    ...
    for vehicle_row in vehicle_rows:
        if len(vehicle_row) > vehicle_amount_idx:
            ...
            # Check if there's a match in GovPay
            if amount_key in govpay_by_amount:
                # Left join: for each matching GovPay row, create a merged row
                for govpay_row in govpay_by_amount[amount_key]:
                    merged_row = vehicle_row + govpay_row
                    merged_rows.append(merged_row)
                matched_count += 1
            else:
                # No match: add vehicle row with empty GovPay columns
    ...
    print(f"\n{'=' * 60}")
    print(f"SUCCESS! Merged file saved: {OUTPUT_FILE}")
    print(f"{'=' * 60}")
